...
With this SDK, you can scan and configure Blue Charm Beacons' "KBeacon" line of devices. The SDK includes the following main classes:
...
in this stage, KBeaconsMgr will scan and parse the advertisement packet of Blue Charm Beacons' KBeacon devices, and it will create "KBeacon" instance for every founded devices, developers can get all advertisements data by its allAdvPackets or getAdvPacketByType function.

**Connection Stage**

After a Blue Charm Beacons' KBeacon is connected, the developer can make some changes of the device by modifyConfig.


## 2. Android demo
To make your development easier, we have two android demos in github. They are:  
* eddystonedemo: The app can scan Blue Charm Beacons' KBeacon devices and configure Eddystone URL, TLM, UID related parameters. This SDK are introduced with reference to this demo.
* ibeacondemo: The app can scan Blue Charm Beacons' KBeacon devices and configure iBeacon related parameters.
...
![avatar](https://github.com/BlueCharmBeacons/BlueCharm_KBeaconDemo_Android/blob/master/bundlesnapshot.png?raw=true)
...
The app can modify the basic parameters of a Blue Charm Beacons' KBeacon through the KBCfgIBeacon class. The KBCfgIBeacon has follow parameters:
uuid: iBeacon uuid
...
The app can modify the eddystone parameters of Blue Charm Beacons' KBeacon through the KBCfgEddyURL and KBCfgEddyUID class.  
...
#### 4.3.4 Update trigger parameters
 For some Blue Charm Beacons' KBeacon devices that have a motion sensor, push button, etc., the app can set advertisement trigger and the device will advertise when the trigger condition is met. The trigger advertisement has follow parameters:
 ...
 *	Trigger advertisement interval: The Bluetooth advertisement interval for trigger advertisement.  You can set a different value from always broadcast.  
 ...
 Example 1:  
  ...
  &nbsp;&nbsp;Trigger adv duration: 30 seconds  
	...
	![avatar](https://github.com/BlueCharmBeacons/BlueCharm_KBeaconDemo_Android/blob/master/only_adv_when_trigger.png?raw=true)
 ...
 Example 2:  
   ...
   &nbsp;&nbsp;Trigger adv duration: 30 seconds  
 	...
 	![avatar](https://github.com/BlueCharmBeacons/BlueCharm_KBeaconDemo_Android/blob/master/always_adv_with_trigger.png?raw=true)
...
Some Blue Charm Beacons' KBeacon can start broadcasting when they detect motion. Also the app can setting the sensitivity of motion detection.  
**Notify:**  
...
#### 4.3.5.1 Ring device
 For Blue Charm Beacons' KBeacon devices that have buzzer function, the app can ring device. For ring command, it has 5 parameters:
